...
export class CrudController {
  ...
  async obtenerDescripcionProyectos(
    @Param('id') id: string,
    @Res() res
  ) {
  try {
    const descripcion = await this.procesosService.obtenerDescripcionProyectos(id);
    return res.status(HttpStatus.OK).json({ ok: true, descripcion });
    } catch (error) {
    return res.status(HttpStatus.BAD_REQUEST).json({ ok: false, error: error.message });
    }
  }
